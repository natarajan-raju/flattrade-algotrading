...
module.exports = createCoreService('api::purge.purge',({ strapi }) => ({
    ...
    async deletePurgeTableMonthly() {
        ...
        //     strapi.webSocket.broadcast({ type: 'action', message: `No data older than 1 month to purge from the purge table.`, status: false });
        //     return;
        // }
        ...
        strapi.webSocket.broadcast({ type: 'action', message: `${oldPurgedRecords.length} records purged from the purge table...`, status: true });
        strapi.log.info(`${oldPurgedRecords.length} records purged from the purge table...`);
    },
